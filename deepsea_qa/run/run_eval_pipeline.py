...
用途说明：
端到端评估流水线 CLI 壳：
对问答数据集进行端到端评估，计算THELMA指标、分类指标和时间指标。

示例：
python -m deepsea_qa.run.run_eval_pipeline \
  --dataset "qa_post/qa_sampled_dataset.jsonl" \
  --llm_provider zhipu --llm_model glm-4-plus --limit 10
"""
...
def _parse_args():
    ...
    p.add_argument("--dataset", type=str, default=str(paths.QA_DATASET_PATH))
...
def main():
    ...
    # 为不同模型创建不同目录
    model_name = args.llm_model.replace('/', '_')  # 替换模型名称中的斜杠，避免路径问题
    qa_output_dir = Path("qa_eval_output") / model_name
    qa_output_dir.mkdir(exist_ok=True, parents=True)
    ...
    if start_idx == 0 and not args.resume:  # 只有当用户没有指定start_from且没有指定resume时才自动检查
        ...
        if qa_output_dir.exists():
            result_files = list(qa_output_dir.glob("qa_eval_results_*.json"))
            if result_files:
                # 按修改时间排序，获取最新的文件
                ...
                with open(latest_file, 'r', encoding='utf-8') as f:
                    try:
                        ...
                        # 只有当使用完整数据集时才自动续传
                        if "qa_sampled_dataset.jsonl" in args.dataset:
    ...
    for i, item in enumerate(dataset[start_idx:], start=start_idx):
        ...
        try:
            ...
            if (i + 1) % 5 == 0:
                ...
                # 保存合并后的结果
                intermediate_path = qa_output_dir / f"qa_eval_results_{timestamp}_intermediate.json"
    ...
    # 加载已有的结果（如果存在）
    existing_results = []
    if auto_resume or args.resume or start_idx > 0:
        result_files = list(qa_output_dir.glob("qa_eval_results_*.json"))
        if result_files:
            latest_file = max(result_files, key=lambda f: f.stat().st_mtime)
            with open(latest_file, 'r', encoding='utf-8') as f:
    ...
    # 保存详细中间结果
    intermediate_path = qa_output_dir / f"qa_intermediate_results_{timestamp}.json"
    with open(intermediate_path, 'w', encoding='utf-8') as f:
        json.dump(intermediate_results, f, ensure_ascii=False, indent=2)
    print(f"详细中间结果已保存至: {intermediate_path}")
    
    # 保存最终结果（与单独评估相同的格式）
    final_path = qa_output_dir / f"qa_eval_results_{timestamp}.json"
    with open(final_path, 'w', encoding='utf-8') as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)
    print(f"最终结果已保存至: {final_path}")
    
    # 保存为Excel（用于文章）
    excel_path = qa_output_dir / f"qa_eval_results_{timestamp}.xlsx"
    ...
    temp_intermediate_files = list(qa_output_dir.glob("*_intermediate.json"))
